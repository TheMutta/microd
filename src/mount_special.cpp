...
void mount_specialfs() {
	...
	if (mount("none", "/dev", "devtmpfs", MS_NOSUID, "") !=0 ) {
		util::warning("Failed to mount /dev. The system may not behave correctly");
	} else {
		util::ok("/dev mounted");
	}
	...
	if (mount("none", "/sys", "sysfs", MS_NOATIME | MS_NODEV | MS_NOEXEC | MS_NOSUID | MS_RELATIME, "") !=0) {
		util::warning("Failed to mount /sys. The system may not behave correctly.");
	} else {
		util::ok("/sys mounted");
	}
	...
	if (mount("none", "/proc", "proc", MS_NOATIME | MS_NODEV | MS_NOEXEC | MS_NOSUID | MS_RELATIME, "") !=0 ) {
		util::warning("Failed to mount /proc. The system may not behave correctly.");
	} else {
		util::ok("/proc mounted");
	}

	printf("Mounting tmpfs in /tmp...\n");
	if (mount("none", "/tmp", "tmpfs", MS_NOATIME | MS_NODEV | MS_NOSUID, "") !=0 ) {
		util::warning("Failed to mount /tmp. The system may not behave correctly.");
	} else {
		util::ok("/tmp mounted");
	}
